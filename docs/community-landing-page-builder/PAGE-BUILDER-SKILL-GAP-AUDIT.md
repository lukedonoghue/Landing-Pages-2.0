...
| Claude capability | Claude source | Current source and status | Audit decision |
|---|---|---|---|
| Brand-derived palette, production typography, and correct logo variant | `C/page-builder/RULES-LOCKED.md:97-117`; `C/design-system-generator/SKILL.md:93-105,425-436` | Brand extraction exists at `N/SKILL.md:87-90` and `N/references/research-and-claims.md:56-58`, but design direction is only broad at `N/SKILL.md:129-135` - **Partial** | Add directly: extract and use the supplied production page's actual heading and body fonts by default, plus a compact brand-fidelity block. |
