  ...
  try {
    ...
    for (const viewport of report.viewports) {
      ...
      try {
        ...
        const submittingLabel = await page.locator(`${fixture.selectors.modal} form[data-lead-form]`).first().getAttribute('data-submitting-label').catch(() => '');
        if (submittingLabel && fixture.selectors.error) {
          // Capture dynamic form wording with a local rejection that cannot write a lead.
          let releaseFailure;
          ...
          await page.locator(fixture.selectors.submit).first().filter({ hasText: submittingLabel }).waitFor();
          await capture('modal', 'pending', fixture.selectors.modal);
          releaseFailure();
          await page.locator(fixture.selectors.error).waitFor({ state: 'visible' });
          await capture('modal', 'failure', fixture.selectors.modal);
        }
        phase = 'thank-you';
        if (allowWrites) {
