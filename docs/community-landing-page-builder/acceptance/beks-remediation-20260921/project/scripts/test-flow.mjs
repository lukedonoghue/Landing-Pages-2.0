...
const siteConfig=JSON.parse(readFileSync('src/site-config.json','utf8'));
...
try{
  ...
  assert.ok(metrics.totals.leads>=1);
  if(siteConfig.analyticsMode==='disabled'){
    assert.equal(metrics.totals.visitors,0);assert.equal(metrics.totals.conversions,0);assert.equal(metrics.totals.conversion_rate,0);
  }else{
    assert.ok(metrics.totals.visitors>=1);assert.ok(metrics.totals.conversions>=1);assert.ok(metrics.totals.conversion_rate<=100);
  }
  ...
  const metricCheck=siteConfig.analyticsMode==='disabled'?'disabled optional analytics remain zero':'visitor/conversion metrics';
  const report={status:'pass',tested_at:new Date().toISOString(),url:base.href,mode:'local-d1',receipt_id:receipt.receipt_id,lead_id:receipt.lead_id,checks:['unauthorized access','browser form','stored receipt','brochure response','login','CRM status','note persistence',metricCheck,'responsive admin','logout revocation'],metrics:metrics.totals};
