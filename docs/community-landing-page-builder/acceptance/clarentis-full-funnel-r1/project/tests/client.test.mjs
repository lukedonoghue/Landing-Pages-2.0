...
test('consent UI can be delegated or disabled without trusting a stale stored grant', async () => {
  for (const consentUi of ['external', 'disabled']) {
    ...
    state.funnel.setConsent(true);
    assert.equal(state.funnel.privacyState().measurement_allowed, consentUi === 'external');
    assert.equal(state.funnel.privacyState().attribution_allowed, consentUi === 'external');
  }
