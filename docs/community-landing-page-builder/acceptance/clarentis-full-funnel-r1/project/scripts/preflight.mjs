...
import {hasUnfinishedStarterContent} from './preflight-content.mjs';
const failures=[];
...
function scan(dir){
 for(const entry of readdirSync(dir,{withFileTypes:true})){
  ...
  if(/\.(html|js|json|txt)$/.test(p)){
   ...
   if(p.endsWith('.html')&&hasUnfinishedStarterContent(text))failures.push(`${p} contains unfinished starter content.`);
