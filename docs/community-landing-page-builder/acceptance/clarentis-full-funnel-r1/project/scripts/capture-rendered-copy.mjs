...
import { readFileSync, mkdirSync, realpathSync, lstatSync, existsSync } from 'node:fs';
...
export async function runCopyCapture(args) {
  ...
  const jsonMaster = inside(root, 'build/page-copy.json');
  const masterPath = existsSync(jsonMaster) ? jsonMaster : inside(root, 'build/page-copy.md');
