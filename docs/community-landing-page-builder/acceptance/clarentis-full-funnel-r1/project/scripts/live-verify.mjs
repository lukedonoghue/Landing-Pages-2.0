  ...
  try {
    ...
    if (!publicProof) {
      ...
      if(fixture.pdf_path){
        ...
        brochure=foundPdf && pdfResponse.ok() && new URL(pdfResponse.url()).origin===target.url.origin && (await pdfResponse.body()).subarray(0,5).toString()==='%PDF-';
    ...
    report.journey_assertions.brochure=check(report,fixture.pdf_path?'Brochure download works after submission':'No brochure is required for this offer',publicProof.brochure);
