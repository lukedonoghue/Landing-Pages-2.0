...
class FunnelParser(HTMLParser):
    def __init__(self) -> None:
        ...
        self.image_section_indexes: list[int] = []
    ...
    def handle_starttag(self, tag: str, attrs) -> None:
        ...
        if tag == "img":
            ...
            self.image_section_indexes.append(sum(1 for ancestor_tag, _ in ancestors if ancestor_tag == "section"))
        if tag == "section":
...
def main() -> int:
    ...
    funnel_config_path = project_root / "funnel.json"
    funnel_config = json.loads(funnel_config_path.read_text()) if funnel_config_path.is_file() else {}
    if funnel_config.get("quality", {}).get("complete_workflow"):
        minimum_sections = funnel_config.get("quality", {}).get("minimum_sections", 0)
        images_config = funnel_config.get("images", {})
        minimum_images = images_config.get("minimum_rendered_assets", 0) if images_config.get("enabled", True) else 0
        checks["landing_page_sections"] = len(index.sections)
        checks["rendered_landing_images"] = len(index.images)
        if len(index.sections) < minimum_sections:
            failures.append(f"Landing page renders {len(index.sections)} sections; complete funnel requires at least {minimum_sections}")
        if len(index.images) < minimum_images:
            failures.append(f"Landing page renders {len(index.images)} images; image-enabled funnel requires at least {minimum_images}")
        if minimum_images and (not index.image_section_indexes or index.image_section_indexes[0] > 1):
            failures.append("The first decision section/hero has no rendered image")
