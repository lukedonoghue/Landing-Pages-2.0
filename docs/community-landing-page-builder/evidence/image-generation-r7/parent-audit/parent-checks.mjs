...
const root = '/Users/mac/Documents/Codex/2026-09-17/co/work/community-acceptance-20260920/runs/image-generation-r7/project';
...
try {
  for (const file of ['index.html', 'styles.css', ...((await readdir(`${root}/publish/assets`)).map(f => `assets/${f}`))]) {
  ...
  for (const [width,height] of [[320,700],[390,844],[768,1024],[1024,800],[1280,600],[1440,900]]) {
    ...
    await page.goto('file:///Users/mac/Documents/Codex/2026-09-17/co/work/community-acceptance-20260920/runs/image-generation-r7/project/publish/index.html', {waitUntil:'networkidle'});
    await page.evaluate(() => {
      window.__intents = [];
    ...
    check(`Early phone ${width}`, await page.locator('.header-actions .phone-link').evaluate(e => {
    ...
    check(`Four distinct image placements ${width}`, await page.locator('figure img').evaluateAll(es => es.length >= 4 && new Set(es.map(e => e.currentSrc.replace(/-(650|800|1100|1600)\.webp$/,'.webp'))).size === es.length));
