...
const root = '/Users/mac/Documents/Codex/2026-09-17/co/work/community-acceptance-20260920/runs/image-generation-r6/project';
...
try {
  for (const file of ['index.html', 'styles.css', 'script.js', ...((await readdir(`${root}/publish/assets`)).map(f => `assets/${f}`))]) {
    check(`Published bytes match ${file}`, hash(await readFile(`${root}/${file}`)) === hash(await readFile(`${root}/publish/${file}`)));
  }
  for (const [width,height] of [[320,700],[390,844],[768,1024],[1024,800],[1280,600],[1440,900]]) {
    const page = await browser.newPage({viewport:{width,height}});
    const errors = [];
    page.on('pageerror', e => errors.push(e.message));
    await page.goto('file:///Users/mac/Documents/Codex/2026-09-17/co/work/community-acceptance-20260920/runs/image-generation-r6/project/publish/index.html', {waitUntil:'networkidle'});
    await page.evaluate(() => {
      window.__intents = [];
    ...
    check(`Early phone ${width}`, await page.locator('.header-contact .phone-link').evaluate(e => {
    ...
    for (const link of await page.locator('[data-primary-action]').all()) {
      ...
      check(`Keyboard quote action ${width}`, await page.evaluate(() => {
        const url = new URL(window.__intents.at(-1));return url.protocol === 'mailto:' && url.pathname === 'info@clarentis.co.uk' && url.searchParams.get('subject') === 'Fixed-fee quote request';
      }));
    }
    ...
    for (const figure of await page.locator('figure').all()) {
      ...
      const caption = figure.locator('figcaption');await caption.scrollIntoViewIfNeeded();await page.waitForTimeout(250);
      captions.push(await caption.evaluate(e => {
        ...
        return {text:e.textContent,top:r.top,bottom:r.bottom,height:innerHeight,visible:r.top>=-1&&r.bottom<=innerHeight+1&&r.width>0&&s.visibility!=='hidden'&&s.display!=='none',coveredByImage:top?.tagName==='IMG'};
      }));
    }
    ...
    check(`Four distinct image placements ${width}`, await page.locator('figure img').evaluateAll(es => es.length >= 4 && new Set(es.map(e => e.currentSrc.replace(/-(720|1100|1440)\.webp$/,'.webp'))).size === es.length));
