...
2026-09-21 completion: user asked to finish the review, not launch another build. Garden Room R6 review is now complete; acceptance remains blocked on option/research-path coverage, mobile-menu focus, narrow phone size and preview-state copy. Native-dialog focus warning was a false positive and is cleared. See EXTERNAL-REFERENCE-R6-FINAL-REVIEW.md. Source corrections are in existing references; original page unchanged. At 70% used, stop new builds and preserve the deployment/CRM reserve.

Latest checkpoint: Garden Room external-reference attempt 6 completed. Parent verified modal close after validation and seven distinct image placements, but found a 13px narrow phone despite the pre-launch 14px rule. Source now adds computed-phone measurements to the existing browser pass and narrow spot capture. All 71 package tests and twelve browser fixtures pass. See EXTERNAL-REFERENCE-R6-REVIEW.md for evidence, calibration limits and resume order. Usage reached 69% used, with a 70% absolute cap; no further full build before authorization. Current artifact is not accepted as a clean first-run pass. PDF, grouped-field variants, full fresh visual review, CRM and deployment remain incomplete.

Latest progress: Garden Room external-reference attempt 6 launched at 22:52 Kyiv from source `2bf9090`, after complete Clarentis attempt-7 audit. Four-image relevance/no-reuse and native renderer behavior now pass; 107 parent assertions pass. Remaining narrow-phone and FAQ wording warnings were corrected in source before the new freeze. See IMAGE-GENERATION-R7-REVIEW.md and CASE2-CARRYOVER-AUDIT.md. Latest usage 66% used; absolute cap 70%. Parent continues the wait/audit/correct loop in the active turn. All 71 tests pass. No claim that all businesses, modal/backend cases or PDF scope have passed. One builder active, Sol 5.6 xhigh, no context fork or prior audit supplied.
