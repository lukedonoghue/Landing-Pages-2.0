...
export function runBackup(args, { run = spawnSync, log = console.log } = {}) {
  ...
  if (plan.action === 'restore-plan') {
    ...
    log(`Recovery plan for ${plan.file}:\n1. Run backup verify first (an isolated local restore).\n2. Create a NEW empty D1 recovery database in the intended Cloudflare account.\n3. Import using: wrangler d1 execute RECOVERY_DATABASE --remote --file <this private SQL file>\n4. Apply any current application migrations missing from the imported backup to this recovery database.\n5. Delete sessions from the recovery database and rotate the owner password there before access.\n6. Bind a separate preview Worker, check counts, login, lead detail and analytics.\n7. After the owner confirms the exact recovered database and cutover, update the production DB binding and deploy. Keep the original DB for rollback.\nThis command makes no database changes. Never import directly over the live client database.`);
    return plan;
  }
  if (plan.dryRun) {
