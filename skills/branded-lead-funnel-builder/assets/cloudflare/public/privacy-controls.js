...
(() => {
  ...
  function render(){
    ...
    const optional=state.analytics_mode!=='disabled'||state.attribution_mode==='consent';
    description.textContent=state.analytics_mode==='disabled'?'Visitor measurement is disabled on this site.':'Optional measurement counts page visits and enquiries without sending contact details to analytics.';
    ...
    stateText.textContent=state.loading?'Loading privacy settings. Optional data is off.':!state.configured?'Privacy settings are unavailable. Optional data is off.':state.browser_opt_out?'Your browser privacy preference keeps optional data off.':state.measurement_allowed?'Optional measurement is on.':state.attribution_allowed&&state.attribution_mode==='consent'?'Optional campaign details are allowed.':'Optional measurement is off.';
