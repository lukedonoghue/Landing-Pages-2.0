...
LABELS = {
    ...
    "publishing_setup": "Prepare the publishing destination",
    "awaiting_publish_approval": "Review the finished funnel",
...
def inspect(root):
    ...
    if approval["status"] not in PASS:
        ...
        return at(
            ...
            "Present the full current copy and record the user’s actual approval when supplied. Reuse an existing approval only for its unchanged copy/offer revision.",
    ...
    if not configured:
        return at(
            "publishing_setup",
            "Prepare the intended Cloudflare account/site using the existing authorized setup scope. Keep the approved copy. Configuration changes need a fresh QA snapshot before final publishing approval.",
        )
    ...
    if publish["status"] not in PASS:
        ...
        if not publish["failures"] or not all(
            ...
            return at(
                "local_verification",
                "Copy or quality evidence changed during the readiness inspection. Resolve these findings and inspect again before asking for final approval.",
                status="blocked",
            )
        return at(
            "awaiting_publish_approval",
            "Present this exact finished funnel, destination and verification scope. Record the actual final approval if it has not already been given for this revision.",
            status="awaiting_review",
        )
    report["completed"].append("publish_approved")
    return at(
        "ready_to_publish",
