...
Known recovery boundaries: if a form might already have submitted, its request key/receipt is saved and another submission is blocked; automatic continuation of that partial CRM journey is not implemented yet. An unconfirmed migration, missing upload origin, changed active version, legacy deployment without a sealed release, or failed release needing a corrected replacement currently requires diagnosis/reconciliation. Do not delete the release pointer, fabricate reports, repeat a submission or reupload as a workaround. Full superseding/reconciliation and an authorized real Cloudflare pilot remain B03/B18/B19 acceptance work.
