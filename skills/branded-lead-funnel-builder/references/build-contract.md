...
Do not hash or transmit contact data for built-in reporting: its event contains only a receipt ID. Current helpers contain delivery errors separately from analytics; preserve them. The UI locks the pending request's fields and replays the same contact details/key when the response is uncertain, while removing optional metadata that has since been withdrawn, then unlocks only after a definite validation rejection. The accessible error explains the uncertainty.
