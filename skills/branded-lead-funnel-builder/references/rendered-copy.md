...
Capture visits 1440px desktop and 390px mobile layouts, opens visible native FAQ disclosures, reads each form step and loads the thank-you page. It preserves inline emphasis and excludes hidden/clipped substitutes. Authored input placeholders and dropdown option labels are included; entered form values are excluded. Custom accordions/tabs need exact CSS selectors in the fixture's `copy_interactions` list; review the selectors before running them. The default capture blocks mutating requests and submits no lead. The required local journey separately proves actual submission and CRM persistence.
