...
The Free default is manual secure-link delivery. With a valid HTTPS `CRM_PUBLIC_ORIGIN` and no configured sender, an authenticated Admin can create invitations, owner-email confirmations and approved password-reset actions for any valid inbox, including Gmail. The response includes the recipient, subject, expiration and one-use URL. D1 stores only the keyed token hash. The UI offers Copy link and Open email app; it never exposes the link publicly. The action URL expires, cannot be reused and is bound to the current account/request version.

Automatic Cloudflare email is optional. It requires a configured sender-domain binding and an authorized verified recipient. Automatic delivery is at-least-once across reset-approval lease recovery. If a provider call outlives its 90-second claim, an administrator may retry and the provider may deliver both messages. An action becomes usable only when its post-send database transition still owns the current request claim; a superseded action remains failed, so only the latest approved reset token can be used. Operators should tell recipients to use the latest message and should not treat provider acceptance as proof of inbox delivery.
...
Account-action email delivery is at-least-once at the provider boundary when an uncertain send is explicitly retried; provider acceptance is not inbox delivery. Tokens are still single-use and version-bound. Manual links are returned only to an authenticated administrator, never logged as sent email. A manual reset of the acting administrator is denied: use current-password verification or a different administrator.

Lead deletion and cancellation of its pending delivery jobs commit atomically; repeated deletion reconciles the same soft removal. The queue excludes undeliverable jobs before applying its batch limit. Terminal submission conflicts carry `submission_removed` or `submission_conflict`; the visitor must explicitly start a new enquiry. Unknown failures retain the same idempotency identity.
