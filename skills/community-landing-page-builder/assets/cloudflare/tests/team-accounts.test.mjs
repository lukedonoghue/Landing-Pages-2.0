...
let mf,db,ownerCookie,sequence=0;
...
before(async()=>{
  ...
  const bundle=await build({stdin:{contents:`import worker from './src/worker.js'; export default {async fetch(request,env,ctx){
    ...
    return worker.fetch(request,{...env,CRM_VERIFIED_RECIPIENTS:JSON.stringify(verified.results.map(row=>row.email)),EMAIL:{send:async message=>{
...
test('email failures and expired links never activate accounts',async()=>{
  ...
  const token=await lastToken('failed@example.invalid');
...
test('free-only recipient guard blocks unverified mail without calling the provider',async()=>{
  ...
  assert.equal((await db.prepare("SELECT status FROM crm_users WHERE username='unverified'").first()).status,'invited');
});
