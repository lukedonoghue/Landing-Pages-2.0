...
(() => {
  ...
  let loading=true,configured=false,mode='disabled',attributionMode='disabled',advertisingUserDataMode='disabled',sensitiveCategory=false,gtmContainerId='',gtmLoaded=false,choice=null,epoch=0,storageUsable=true;
  ...
  const status=()=>({configured,loading,analytics_mode:mode,attribution_mode:attributionMode,advertising_user_data_mode:advertisingUserDataMode,sensitive_category:sensitiveCategory,choice,measurement_allowed:canMeasure(),attribution_allowed:canAttribute(),customer_data_allowed:canUseCustomerData(),browser_opt_out:browserOptOut(),can_persist:storageUsable||cookieChoice()!==null});
  ...
  ready=fetch('/api/privacy-config',{cache:'no-store',credentials:'same-origin',signal:policyController.signal})
    .then(async response=>{if(!response.ok)throw Error();const value=await response.json();if(!['consent','essential','disabled'].includes(value.analytics_mode)||!['consent','lead','disabled'].includes(value.attribution_mode)||!['consent','disabled'].includes(value.advertising_user_data_mode||'disabled'))throw Error();mode=value.analytics_mode;attributionMode=value.attribution_mode;advertisingUserDataMode=value.advertising_user_data_mode||'disabled';sensitiveCategory=value.sensitive_category===true;gtmContainerId=typeof value.gtm_container_id==='string'?value.gtm_container_id:'';serverOptOut=value.browser_opt_out===true;configured=true;})
    .catch(()=>{configured=false;mode='disabled';attributionMode='disabled';advertisingUserDataMode='disabled';sensitiveCategory=false;gtmContainerId='';})
    .finally(()=>{loading=false;clearTimeout(policyTimeout);reconcile();captureTouch();visit();loadGtm();signal();});
