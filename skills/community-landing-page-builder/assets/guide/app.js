(() => {
  ...
  function render(data){current=data.next;const key=JSON.stringify([current.revision,current.stage,current.kind,current.review_fingerprint]);$('#state').textContent=data.running?'Working on this step. You can leave a question for the active agent.':current.kind==='complete'?'Selected scope complete':current.kind==='paused'?'Progress saved':'Ready for the next action';
    ...
    if(current.kind==='question'){
      ...
      const changed=current.approval_kind==='copy'?(current.changed_passages||[]):[];
      if(changed.length)$('#actions').append(button('Show what changed ('+changed.length+')',async()=>{const r=await api('/api/review');$('#documents').replaceChildren();for(const p of r.passages.filter(p=>p.status!=='unchanged')){$('#documents').append(create('h3',p.id.replace('section:','Section: ')),create('pre',p.text));}$('#review').open=true;}));
