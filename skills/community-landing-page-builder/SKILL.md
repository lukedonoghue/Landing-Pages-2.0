...
8. **Guide and thank-you.** Include every approved `brochure.text` passage in `build/guide.json`, run `build_guide.py`, review every page, then `thank_you_page.py`.
...
- **Advertising and GTM:** [references/advertising-tracking.md](references/advertising-tracking.md). For Google Ads without a tag, the CRM lead export (`/api/admin/leads/export.csv`) now includes `gclid`, `gbraid`, `wbraid`, `utm_source`, `utm_campaign` and `utm_term`, so qualified leads can be uploaded as offline conversions. Keep raw contact data out of analytics. The optional contract uses `customer_data_ready` for consented provider-normalized hashes and `lead_accepted` for the confirmed conversion receipt. Google is primary; Meta and Microsoft require destination-specific verification.
