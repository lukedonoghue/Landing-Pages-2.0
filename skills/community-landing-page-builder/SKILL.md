...
## Field-tested fast path (read first)

This is the exact command path that reaches a local final, verified end to end on 26 September 2026 (see `docs/community-landing-page-builder/E2E-FIELD-TEST-FIX-SPEC-20260926.md` in the repository). The agent runs every command; the owner only answers business questions and approvals. `PROJECT` is a new folder outside the skill checkout.

1. **Start and research.** `python3 scripts/guide.py start PROJECT --mode guided --goal preview --website URL --name NAME`. Capture the official pages as files under `PROJECT/research/sources/` (text plus HTML), then write `build/discovery.json` and run `guide.py discover`. Suggest the public phone number (`phone`) when the site publishes one.
2. **Ask only what research cannot settle.** `guide.py next` returns at most three questions. For an enquiry page this includes the Google Ads keywords or ad headline (`keywords`), so the headline repeats the searcher's words; "not sure" is a valid answer and the agent then infers intent from the site and labels it inferred. Record answers with `guide.py answer`, then the brief approval with `guide.py approve`.
3. **Local transitions.** When `guide.py next` reports `kind: local`, run `python3 scripts/guide.py local PROJECT` (or `workflow_runner.py claim PROJECT`, which now performs local operations itself). After the lead-inbox scaffold, install the project tools once: `python3 scripts/quickstart.py bootstrap --project .` from `PROJECT`.
4. **Evidence, once.** Write `build/strategy-brief.md` and `build/claim-ledger.md` (they also satisfy `docs/RESEARCH-BRIEF.md` and `docs/CLAIM-LEDGER.md`), `build/research-acceptance.json`, `build/reference-fidelity.json`, and the remaining `docs/*.md` required by `process_contract.py`. Point image-attempt evidence at captured source HTML, never at `image-plan.json`.
5. **Copy.** Structured `build/page-copy.json` per `references/copy-workflow.md`. Put every custom interface string (header trust line, badges, microcopy) in `interface_text`; modal state copy uses `failure` and `uncertain`; keep writer notes and edit logs in a sidecar file, not in the master. Then the editorial review, `copy_acceptance.py verify`, `copy_library.py audit` and the owner copy approval.
6. **Images.** `image_workflow.py init`, `inventory-html`, `acquire`, `optimize` (CMYK is handled), `add-asset` for a new slot. Use `--rights local-preview-only` for any image whose reuse is not yet authorised; handoff and publication refuse it.
7. **Build, then compare with Blue Mountain.** Build `public/`, capture `--phase initial`, run the control comparison and repairs, and review every image render before the final capture.
8. **Guide and thank-you.** The guide's reader text (title, chapters, checklist, next step) lives once, in `brochure.text` of the copy master; the scaffolded `build/guide.json` sets `content_source` and holds only delivery, brand, sources and images. Run `build_guide.py` (it derives the rest), review every page, then `thank_you_page.py`.
9. **Verify everything with one command.** From `PROJECT`: `python3 scripts/quickstart.py verify --project .`. It starts the local Worker, snapshots once, runs static, browser (10 viewports), performance, Chromium/WebKit, local form-to-CRM journey and rendered-copy checks, assembles the image, copy and catalogue reports, records every gate and prints the remaining next actions (normally the fresh visual review and control acceptance). `python3 scripts/quickstart.py preview --project .` serves the page and private lead inbox locally.

Order matters: finish image reviews, guide review and every copy change before the final control capture and the verify snapshot. Editing `docs/` no longer invalidates rendered evidence, but any change to `public/`, `funnel.json` or canonical inputs does.
...
1. **No Unicode long dashes.** Never use U+2014, U+2013, or their named or numeric HTML equivalents in generated customer-facing content. Rewrite the sentence or use an ASCII hyphen. In a verbatim customer review, replace a long dash with a spaced hyphen as typographic normalisation and keep the review; do not drop genuine proof because of punctuation. Run `scripts/scan_surfaces.py` before delivery and after any copy change.
2. **No invented proof.** Use only supported claims, numbers, ratings, awards, guarantees, testimonials, people, locations, credentials, and proof imagery. Preserve qualifiers. Omit unsupported proof. Every factual sentence, including buyer-education prose in `brochure.text` and FAQ answers, must trace to a captured source or the owner's instruction; plausible general knowledge (how a test works, what a quote contains, what a regulator requires) is not a source.
...
11. **A useful PDF is a required build stage.** Create a concise, researched guide, service overview, options summary, checklist, catalogue or verified price list appropriate to the buyer. Complete `build/guide.json`, run `python3 scripts/build_guide.py .`, inspect every rendered page and keep the matching `data-guide-embed` preview plus `data-guide-download` fallback on the thank-you page. A placeholder, missing, unrendered or undelivered PDF blocks final QA. Omit it only when research establishes a concrete reason it would not help this buyer, recorded with supporting sources in the strategy brief and `funnel.json`. Lack of a supplied PDF, no explicit request, limited time or a page-only default is not a reason. Keep it secondary unless research supports a download-led offer. Delivering it on the thank-you page after an enquiry is the default lead magnet (name it in the offer box); an additional ungated download is optional, and never make a guide the only way to reach the enquiry.
12. **Keep text work on the active AI provider.** Never require, install, authenticate, or invoke a second provider merely to write or review copy. Follow [platform-native model routing](references/model-routing.md): use the strongest suitable writing model available on the active provider for the draft and a separate fresh-context review on the same provider (`config/routing.json` holds the current defaults; never downgrade a stronger session model just to match a listed default). Record the models that actually ran. Reviewer independence means a fresh context, not a different vendor.
...
Use one exact primary CTA label. For Google Ads lead pages, make it offer-led and name the value received ("Get my free quote", "Request brochure & pricing"); when a guide exists, name it in the CTA or the offer box beside it. Repeat the searcher's keyword in the H1 (ad scent) using the recorded `search_intent`. Explain the real operational reason the offer works. Map important benefits to actual capability or proof. Use truthful urgency only when a sourced deadline, capacity limit, or availability constraint exists. Replace generic hype such as `world-class`, `game-changing`, `seamless`, `robust`, or `innovative` with the specific action, limit, mechanism, or evidence unless the term is genuinely necessary and made concrete.
...
Classify each used image as `decorative`, `proof`, `portrait`, `diagram`, `screenshot`, or `illustrative`: write that role on the rendered `<img>` as `data-image-role`, and record the plan's `trust_class` (`client-proof`, `illustrative`, `decorative`; the role words are accepted as aliases) in `image-plan.json`. Also mark whether its pixels contain information that must remain readable. Diagrams, screenshots, infographics, and text-bearing images must not use destructive `cover` crops and must not sit behind overlapping copy. Text must not cover a person's face or the subject needed to understand the image.
...
Start from the template's benchmark hero (`public/index.html`, modelled on the control page): brand, one short credibility line and the verified phone in a compact header (`npm run configure` fills the phone from `funnel.json`); an outcome headline carrying the ad keyword; one supporting line; one `data-primary-action` button; optional `.hero__media` beside the copy. It already fits the next section's heading into the first screen at every QA viewport, including 1280x600, so adapt its content rather than rebuilding its spacing.
...
- **Advertising and GTM:** [references/advertising-tracking.md](references/advertising-tracking.md). For Google Ads without a tag, set `tracking.google_ads.offline_conversions` in `funnel.json` (the CRM stages that count, each with the exact Google Ads conversion action name and optional value); the CRM then offers a ready-to-upload Google Ads conversions file, and the contacts CSV carries `gclid`, `gbraid`, `wbraid` and UTM terms.
