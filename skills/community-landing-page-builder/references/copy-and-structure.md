...
Run these on the same draft using [platform-native model routing](model-routing.md). They require no user approval, extra provider subscription, classification service or extra research round when the evidence is sufficient. Use a fresh same-provider review context when available; otherwise perform a deliberate second pass in the current agent. Revise affected passages, not the entire page repeatedly.
...
- One page, one main audience, one primary offer, and one exact primary CTA label.
- Match supplied keywords or inferred traffic intent naturally. Do not invent ad-account data.
- Make the H1 concrete enough that a first-time visitor knows the service or offer and desired result.
...
- Never use Unicode long dashes or their HTML entities.
...
- `Get a quote` must collect enough information or clearly state the follow-up.
- Back, Continue, Download, Phone, Email, and Privacy controls may use truthful role-specific labels without becoming competing primary offers.

Use the same offer and follow-up promise in the page, form, success state, thank-you page, and selected brochure. Do not imply email delivery when the file is only available on the thank-you page.
...
Read the rendered page as a cold visitor and as the target buyer. Check:
