...
- every photograph, video, diagram, map, brochure preview, testimonial treatment and interactive moment;
- changes in composition, background, density and pacing that make sections feel distinct.

When several sources exist, use the richest relevant buyer journey. Do not default to the shortest page or a generic template.
...
When source options or specifications help the buyer choose, preserve concrete client-supported differences or a clear verified route to inspect them; a gallery of uses and an invitation to ask are not automatically equivalent. Creating a new brochure is optional, but that does not justify silently dropping a relevant existing brochure or other lower-commitment research path. Preserve its useful role through a working verified destination or equivalent decision information. If its destination cannot be verified, record the limitation rather than inventing a file, promising a download, or marking the job complete merely because the main form works.
...
For a location-dependent service, give an early, qualified geographic-fit cue near the offer or action; keep fuller detail in the FAQ when useful. A registered address or an answer only inside a closed FAQ is weak initial qualification. Verify published coverage or invite a location check without promising unsupported reach; do not turn example project locations into a coverage guarantee. When several genuine customer accounts support distinct important claims, select complementary evidence near those claims or explain the omission. No review-count quota applies, but one budget/design quote does not corroborate unrelated workmanship or delivery claims. Resolve missing material coverage before calling the final table complete.
...
The page should feel at least as complete and intentionally designed as the benchmark while remaining original. Preserve the actual buyer journey, not its wording or pixel design. Use the client brand, better hierarchy, clearer proof, stronger visual roles and more coherent interaction to improve it.

Section count, page height and image count are observations, not targets. A shorter page can pass only when it still carries every relevant decision and feels equally substantial. A longer page can fail when it is repetitive padding.
...
If any answer exposes a regression, revise the page and recapture it. A green technical gate does not override a failed side-by-side review.
