...
This module is the default destination for first-party enquiry forms. Do not ask the owner to choose a CRM or spreadsheet before using it. External CRM synchronization, Sheets and advertising integrations are later optional connections. Use `--static-only` only for an explicit static-only request or a genuinely non-form conversion path.
