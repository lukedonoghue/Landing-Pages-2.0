...
Apply additive migrations through 0006 before the new Worker. Preserve the current owner/password and existing leads. Test each role with separate sessions: direct API denials, stage/note management, denied viewer export/settings, invitation confirmation, expiry/reuse, disabled accounts, admin approval and old-session rejection. Use a synthetic mail sink for automated regression, clearly distinguished from a real inbox test. Keep the existing live login-attempt budget; do not disable throttling for tests.
