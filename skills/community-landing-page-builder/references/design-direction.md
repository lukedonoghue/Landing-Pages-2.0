...
Do not introduce `Inter`, `Roboto`, `Arial`, `Helvetica`, `system-ui`, `Open Sans`, `Lato`, standard `Montserrat`, generic `Poppins`, or `Raleway` as an arbitrary new design default. They remain allowed when first-party evidence verifies them as client typography, and an already verified branded `Inter` must not be replaced merely because it appears in this list. Generic fallback entries may remain after the evidenced primary family for robustness. Record the evidence and rendered result; this rule governs unsupported defaults, not legitimate brand fidelity.
