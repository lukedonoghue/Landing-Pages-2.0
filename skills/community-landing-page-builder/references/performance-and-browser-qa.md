...
Use the project Node version (Node 22.19+ or a supported later release; Node 24 recommended), `npm ci`, then `npx playwright-core install chromium webkit`. If using a bundled runtime, prepend its bin directory to PATH for npm and child commands and verify `node -p 'process.version + " " + process.execPath'` in that environment before the suite. Running one script with an absolute Node path does not change a later npm command's runtime. Missing binaries or unsupported browser runtimes produce **blocked** evidence, never an automatic pass. The browser scripts use Playwright because portable WebKit coverage and a reproducible test runner are required. Interactive exploration can use agent-browser or computer use separately.
...
Protect the user's everyday browser. Prefer the pinned Playwright-managed Chromium/headless shell and WebKit, with isolated temporary profiles, not the installed personal Chrome application. Close contexts and browsers in `finally`, including failed checks; retain the launched PID/profile identity for interrupted-run cleanup. After a timeout, inspect only that run's descendants and stop confirmed abandoned test processes. Never use blanket `killall Chrome`, delete personal profile locks, or access personal browser storage. Old orphaned headless instances of the macOS Chrome application have prevented normal Chrome/profile launches during this project's testing. OAuth is different: open the user's system browser for their manual sign-in, without controlling its private profile.
...
The full verifier checks unauthenticated routes, signs into the admin panel, then submits through the **actual anonymous public form** in a separate browser context. In internal compatibility mode it accepts the built-in choice; in external mode it drives the configured provider adapter; with consent UI disabled it confirms no prompt or optional tag is present. It completes every form step, records the accepted receipt, follows the thank-you navigation, and verifies the advertised PDF download. It retrieves the same lead through the authenticated Worker API, requires the identical receipt and visit event ID, verifies source/device/traffic classification, changes its stage, adds a synthetic-test note, re-reads it, and checks the filtered reporting totals and conversion calculation. It verifies session revocation on logout. For lead-attribution mode, also verify that a fresh tagged submission stores the approved allowlist without an optional-consent choice, while GPC/DNT strips it and no visit/GTM request occurs.

Budget live login attempts across all supplementary UI, attribution and recovery checks: the starter permits eight per IP per 15-minute window, including successful logins. Reuse an authenticated test session for checks that do not test login/logout, reserving attempts for those that do. If a check receives HTTP 429, record throttling as inconclusive, respect Retry-After or the configured window, then retry only the affected check once. Never clear rate-limit records, change IPs or weaken production limits to force acceptance. Password-recovery dry-run evidence does not prove an actual reset; an authorized disposable-demo rotation must additionally verify old-password rejection, old-session revocation and new-password login without exposing either password.
