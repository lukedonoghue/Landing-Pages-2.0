...
The first research task writes `build/discovery.json` with `schema_version: 1`, `input_fingerprint` from the task's `research_input_fingerprint`, `suggestions` as consumed by `guide.py discover`. For new scaffolds with `review_intelligence_version: 1`, that same research task must also complete `research/reviews/review-manifest.json` for the exact business, run `python3 scripts/review_workflow.py compile PROJECT --expected-fingerprint <research_input_fingerprint>`, and leave current `build/review-insights.json` plus `build/testimonial-selection.json`. A researched `no_sources` result is valid; an omitted review search is not. Each suggestion has `id` (a catalog question ID), `source_path`, `source_sha256`, exact `excerpt`, and either `value` or `ambiguous: true` with an explanatory `reason` and optional `{value,label}` options. Suggestions use actual source evidence; do not infer unsupported guarantees, prices, credentials, response times or proof. Review research must keep analysis rights separate from publication rights, never fabricate testimonial identities, and keep Google Maps content provider-dynamic rather than copied into the project. Read the question catalog and controller contract before writing discovery. A changed business identity invalidates its research suggestions. User-confirmed facts remain user evidence, not independent verification.
