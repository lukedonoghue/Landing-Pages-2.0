...
When customer reviews are reasonably available, the same research stage also follows `review-intelligence-and-testimonials.md`: identity-match sources, create `research/reviews/review-manifest.json`, run `review_workflow.py all`, validate the research stage, and pass `build/review-insights.json` plus any publishable testimonial selection into copy work. Missing Google/provider credentials do not block the ordinary local build; use available first-party/user-supplied proof or another proof type.
