...
def render(runtime="both", inherit_models=False):
    ...
    profiles["review-critical"] = {"agent": "lp-reviewer-critical", "tier": "critical", "instruction": p["roles"]["review"]["instruction"], "provider_routes": p["roles"]["review"].get("provider_routes", {})}
    for role, spec in profiles.items():
        ...
        if runtime in {"codex", "both"}:
            routes = spec.get("provider_routes", {}).get("codex", [])
            profile = routes[0] if routes else p["providers"]["codex"].get(spec["tier"], {})
        ...
        if runtime in {"claude", "both"}:
            routes = spec.get("provider_routes", {}).get("claude", [])
            profile = routes[0] if routes else p["providers"]["claude"].get(spec["tier"], {})
