...
UI_TEXT = [
    ...
    # Fixed wording generated by thank_you_page.py and build_reader_guide.py.
...
def expected_surfaces(master, catalogue_enabled):
    ...
    expected = {
        ...
        "thank_you": leaves({key: value for key, value in master.get("thank_you", {}).items() if key != "delivery"}, "/thank_you")
    }
...
def pdf_body(text, page_count, footer_labels=(), running_labels=()):
    pages = text.split("\f")
    ...
    for index, page in enumerate(pages, 1):
        ...
        # Numbered checklist markers added by the PDF renderer are structure, not copy.
        lines = [re.sub(r'^\s*\d{1,2}\.\s+', '', line) for line in lines]
        labels = {normalize(label) for label in footer_labels if normalize(label)}
        running = {normalize(label) for label in running_labels if normalize(label)}
        for end in (0, -1):
            ...
            if lines:
                ...
                if number in {str(index), f"{index:02d}", f"page {index}", f"page {index} of {page_count}", f"{index} / {page_count}"}:
                    ...
                    # Poppler may place the running footer (brand label plus this page's
                    # number) first or last on the page. Remove only that exact footer so a
                    # paragraph continuing across the page break stays contiguous.
                    # Poppler may join an approved footer label and this page's number, and may
                    # place the running brand footer first on the page. Strip the number; move a
                    # running footer out of the text so a paragraph crossing the page stays
                    # contiguous (the approved label is kept once at the end).
                    match = re.fullmatch(r'(.+?)\s+(0?' + str(index) + r')', number)
                    if match and match[1] in running:
                        lines.pop(end); footers.append(match[1])
                    elif match and match[1] in labels:
                        lines[end] = match[1]
                    elif number in running and page_count > 1:
                        footers.append(lines.pop(end))
        cleaned.append("\n".join(lines))
    return "\n".join(cleaned + sorted(set(footers)))


def guide_wording(root):
    """Buyer-facing text of the current reviewed reader guide, as build_reader_guide.py renders it.

    Returns an empty list unless the guide build and its reader review pass for the current
    build, so unreviewed guide text is never treated as approved. Approved brochure.text
    passages remain required in the PDF.
    """
    path = Path(root) / "build/guide.json"
    if not path.is_file():
        return []
    try:
        ...
        return []
    data = read(path)
    words = [data.get(key, "") for key in ("title", "subtitle", "audience", "reader_promise", "next_step", "scope_note", "checklist_title")]
...
def compare(master, funnel, capture, approved_guide=(), footer_labels=None):
    ...
    rows = []
    landing_phrases = [value for _, value in expected["landing"]]
    for surface in ("landing", "modal", "thank_you"):
        ...
        for width in (390, 1440):
            ...
            # thank_you_page.py deliberately reuses the approved landing sections and the reviewed guide.
            shared = landing_phrases + list(approved_guide) if surface == "thank_you" else []
            unexpected = [extra_text(text, phrases + neutral + shared + (state_copy if surface == "modal" and doc.get("state") == "submission-error" else []))
    ...
    if catalogue_enabled:
        ...
        if approved_asset:
            ...
            try:
                ...
                allowed = phrases + neutral + list(approved_guide)
                running = footer_labels if footer_labels is not None else [funnel.get("client", {}).get("name", "")]
                text = normalize(pdf_body(pdf.get("text", ""), pdf.get("page_count", 0), allowed, running))
                missing = [key for key, value in expected["brochure"] if not contains(text, value)]
                unexpected = extra_text(text, allowed)
                if missing:
...
def audit(root, capture_path=None):
    ...
    approved_guide = guide_wording(root)
    funnel = read(root / "funnel.json")
    footers = [funnel.get("client", {}).get("name", "")]
    if (root / "build/guide.json").is_file():
        footers.append(read(root / "build/guide.json").get("brand", {}).get("name", ""))
    result = compare(master, funnel, capture, approved_guide, footers)
    ...
    if approved_guide:
        for name in ("build/guide.json", "build/guide-review.json"):
            artifacts.append({"type": "reviewed_guide_source", "path": name, "sha256": sha(root / name)})
