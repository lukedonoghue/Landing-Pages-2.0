...
export function backupPlan(args) {
  ...
  const booleans=new Set(['--local','--remote','--dry-run','--confirm-legacy-source','--allow-plaintext-temporary']),values=new Set(['--out','--file','--erasure-records','--clean-output']),seen=new Set();
  ...
  if(action==='export'&&args.includes('--remote')&&!args.includes('--dry-run')&&!args.includes('--allow-plaintext-temporary'))throw new Error('Use npm run backup:encrypted for production exports; plaintext requires an explicit --allow-plaintext-temporary exception in a trusted operator shell.');
