...
export function loadFixture(file) {
  ...
  for (const key of ['path', 'thank_you_path', 'pdf_path']) {
...
export async function fillSteps(page, fixture, { inspectField, onStep } = {}) {
  ...
  for (let step = 0; step < 24; step++) {
    ...
    for (const field of await current.locator('input[name],select[name],textarea[name]').all()) {
      ...
      used.add(name);
      if (inspectField) await inspectField(field);
    }
    ...
    try {
      for (const viewport of views) {
        ...
        try {
          ...
          const privacyUiRequired=privacyMode==='internal'&&(privacyState.analytics_mode==='consent'||privacyState.attribution_mode==='consent'||privacyState.advertising_user_data_mode==='consent');
          const privacyTrigger=page.locator('[data-privacy-choices]').first();
          const privacyDialog=page.locator('[data-funnel-privacy-dialog]');
          if(privacyUiRequired){
            await privacyTrigger.waitFor({state:'visible'});await privacyTrigger.click();await privacyDialog.waitFor({state:'visible'});
