...
test('dynamic free email keeps sender restriction and provider-verified destinations',()=>{
  const config={send_email:[{name:'EMAIL',allowed_sender_addresses:['crm@example.invalid']}],vars:{CRM_EMAIL_FROM:'crm@example.invalid',CRM_PUBLIC_ORIGIN:'https://demo.workers.dev',CF_EMAIL_ROUTING_ACCOUNT_ID:'a'.repeat(32)}};
  assert.deepEqual(freePlanFailures(config),[]);
  config.send_email[0].allowed_destination_addresses=['owner@example.invalid'];
  assert.match(freePlanFailures(config).join(' '),/without redeployment/);
  delete config.send_email[0].allowed_destination_addresses;
  config.vars.CF_EMAIL_ROUTING_TOKEN='must-not-be-public';
  assert.match(freePlanFailures(config).join(' '),/Worker secret/);
});
test('usage monitor stays optional and rejects public or broad credentials',()=>{
  ...
  assert.deepEqual(freePlanFailures({vars:{CF_USAGE_PLAN:'workers-free'}}),[]);
  assert.deepEqual(freePlanFailures({vars:{CF_USAGE_PLAN:'workers-paid'}}),[]);
  assert.ok(freePlanFailures({vars:{CF_USAGE_PLAN:'free'}}).length);
  ...
  assert.ok(freePlanFailures({vars:{CF_EMAIL_ROUTING_ACCOUNT_ID:'wrong'}}).length);
});
